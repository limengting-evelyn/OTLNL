...
class KNNClassifier(nn.Module):
    def __init__(self, feat_dim=512, num_classes=1000, feat_type='cl2n', dist_type='l2',device='cuda:0'):
        super(KNNClassifier, self).__init__()
        assert feat_type in ['un', 'l2n', 'cl2n'], "feat_type is wrong!!!"
        assert dist_type in ['l2', 'cos'], "dist_type is wrong!!!"
        self.feat_dim = feat_dim
        self.num_classes = num_classes
        self.centroids = torch.randn(num_classes, feat_dim).to(device)
        self.feat_mean = torch.randn(feat_dim).to(device)
    ...
    def forward(self, inputs, *args):
        ...
        if self.feat_type in ['l2n', 'cl2n']:
            ...
            #inp = np.isnan(inputs.cpu())
            #print("inputs:",True in inp)
            #norm = np.isnan(norm_x.cpu())
            #print("norm_x:",True in norm)
            inputs = inputs / norm_x
            #inp = np.isnan(inputs.cpu())
            #print("inputs:",True in inp)
            #cen = np.isnan(centroids.cpu())
            #print('centroid:',True in cen)
