...
def eval_train(epoch, eval_loader, model, num_all_img, cfeats_EMA, cfeats_sq_EMA, device, queue):    
    ...
    for i in range(args.num_class):
        this_cls_idxs = (total_labels == i).nonzero(as_tuple=True)[0].cpu()
    ...
    ncm_classifier = KNNClassifier(args.feat_size, args.num_class,device=device)
...
if __name__ == "__main__":
    ...
    stats_log=open('./checkpoint_webvision_%s'%(args.imb_factor)+'/model_stats.txt','w') 
    test_log=open('./checkpoint_webvision_%s'%(args.imb_factor)+'/model_acc.txt','w')         
    
    warm_up = 1
    store_name = '_'.join(['webvision', 'InceptionResNetV2', 'exp', str(args.imb_factor)])    
    ...
    """
    net1 = create_model_selfsup(num_classes=args.num_class, device=cuda1)
    net2 = create_model_selfsup(num_classes=args.num_class, device=cuda2)

    net1_clone = create_model_selfsup(num_classes=args.num_class, device=cuda2)
    net2_clone = create_model_selfsup(num_classes=args.num_class, device=cuda1)
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):   
        ...
        if epoch % 10 == 0:
            save_path = f'./checkpoint_webvision_{args.imb_factor}/model_webvision_e{epoch}.pth'
